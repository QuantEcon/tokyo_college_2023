...
def plot_hog():
    ...
    ax.set_title('Hog prices in the US 1924-1959')
    ax.legend()
    ax.grid()
    plt.savefig('hog_prices.pdf')
    plt.show()
...
def plot_supply_demand():
    ...
    plt.savefig("hog_supply_demand.pdf")
    plt.show()
...
def plot_backward_ts(p, filename="hog_backward_ts0.pdf"):
    m = Market()
    ts_plot_price(m, p, filename, ts_length=18)
...
def ts_price_plot_adaptive(model, p0, ts_length=10, α=[0.9, 0.75]):
    fig, axs = plt.subplots(1, len(α), figsize=(10, 5))
    ...
    plt.savefig("hog_adaptive.pdf")
    plt.show()


def plot_price_adaptive():
    m = Market()
